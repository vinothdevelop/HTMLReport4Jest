...
class App extends Component {
  ...
  formatTreeViewData(testResults) {
    ...
    if (testResultData.testResults) {
      [rootElement.children, id] = this.parseTreeData(testResultData.testResults, [], id);
    }
  ...
  parseAncestor(ancestors, testCase, parentArray, id) {
    let ancestorCopy = [...ancestors];
    if (ancestors.length > 0) {
      let itemTitle = ancestors[0];
      let elementIndex = parentArray.findIndex((value) => {
      ...
      if (elementIndex === -1) {
        ...
        nodeValue.numPendingTests = testCase.status === 'pending' ? 1 : 0;
        nodeValue.numTodoTests = testCase.status === 'todo' ? 1 : 0;
        nodeValue.id = `id${id}`
        id++;
        ancestorCopy.shift();
        [nodeValue.children, id] = this.parseAncestor(ancestorCopy, testCase, [], id);
        parentArray.push(nodeValue);
      }
      else {
        ancestorCopy.shift();
  ...
  render() {
    return (<div className="App" >
      <Header menuStateChange={() => this.menuStateChange('open')} />
      <Sidebar
